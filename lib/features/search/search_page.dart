    ...
    return Scaffold(
      ...
      body: _query.isEmpty
          ...
          : CustomScrollView(
              slivers: [
                SliverToBoxAdapter(
                  child: _UsersSection(query: _query),
                ),
                SliverPadding(
...
class _UsersSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return async.when(
      ...
      error: (Object e, _) => Padding(padding: const EdgeInsets.all(16), child: Text('用户搜索失败：$e')),
        ...
        return Column(
          ...
          children: [
            Padding(
              ...
              child: Text('用户 (${users.length})', style: Theme.of(context).textTheme.labelMedium),
            ),
            for (final u in users)
              ListTile(
                leading: Avatar(pubkey: u.pubkey, radius: 18),
                title: Text(u.metadata?.bestName ?? u.pubkey.substring(0, 12),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                subtitle: u.metadata?.about == null
                    ? null
                    : Text(u.metadata!.about!, maxLines: 1, overflow: TextOverflow.ellipsis),
