...
- **媒体加载**（头像/帖子图片/视频）：`CostrNetworkImage` 走带超时的 `flutter_cache_manager`
  ...
  **头像/背景图被墙也能手动走代理**：资料页的大头像与背景图用 `ProxyableNetworkImage`
  （手动、非自动——与帖子「代理媒体」同款逻辑）：先直连原图，被墙类失败（超时/连接被重置，
  404 不算）时在占位图上浮现「代理」小按钮，点按改走代理镜像并**落本地磁盘缓存**（后续秒出）。
  同一 host 一旦学到被墙（按**域名**记忆，会话内生效），其余该域名的头像/背景图直接走代理、
  不再重复 8s 超时。列表里的小头像不带按钮（保持简洁），仍用首字母占位兜底。
