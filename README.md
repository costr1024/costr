...
| 平台 | 版本 | 文件 |
| --- | --- | --- |
| Android | **1.0.1** | [`app-release.apk`](https://github.com/costr1024/costr/releases/download/v1.0.1/app-release.apk)（≈72 MB，universal APK，含 arm64/arm/x86_64/x64 全 ABI） |

> 当前为正式版（v1.0.1）。Android 包 `applicationId = com.costr.costr`，用正式
...
| 语言过滤 | 首页内建 🌐/🇨🇳/🇬🇧/🇯🇵 启发式过滤（前 100 字符主导文字判定，抗混写垃圾帖） | 多无内建语言过滤 |
...
- **标签与语言过滤**：NIP-12 `t` tag + 正文内联 `#hashtag`（支持中文），chip 点选过滤/长按关注。**关注 hashtag 存 NIP-51 kind-10015 + NIP-44 加密 content**（`[["t",…]]` JSON，对齐 Amethyst 的私密兴趣列表，迁移互通），同时只读聚合 kind-30015 命名兴趣集的明文 `t`。语言下拉 🌐/🇨🇳/🇬🇧/🇯🇵，按前 100 字符的主导文字判定（拉丁占比压倒性判英文、谚文判韩文、假名判日文、汉字判中文），巨型混写垃圾帖不会误入其他语言过滤。
