...
- **服务器节点页（设置→服务器节点）**：列出 Costr 连接的所有中继 + 实时连接状态（已连接 / 连接中 / 已断开）。**真实 WebSocket 时延**：`RelayClient.measureRtt` 对每台中继发一个不可能命中的 REQ（`since` 设到 1 年后）并计时到该 subId 的 `EOSE`——relay 无 DB 扫描、立即 EOSE，≈ 网络往返而非 ICMP ping。时延缓存到本地 SQLite（`relay_rtt:<url>`，FIFO 只留最近 3 次），进页面测一次、停留每 5s 自动重测；显示取 3 次平均值（不足 3 次按实际平均），按快/中/慢绿/黄/红着色，可手动「重新测速」。
