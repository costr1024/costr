...
- **用户主页**：点 feed 帖子的头像或作者名 → 打开该用户 profile（`/u/:pubkey`，push 进栈、自带返回）。复用 ProfilePage（参数化 pubkey）：他人主页只展示资料，自己的主页才有 hex pubkey + 登出。主页资料下方 3 个 tab：**帖子**（顶层帖，`!isReply`）、**回帖**（`isReply`，带回复的父帖引用 + 左侧竖线缩进层级，NIP-10 `e` tag 解析）、**关注**（该用户的 NIP-02 kind-3 联系人列表，`userFollowsProvider` 拉，每行头像+名可点进对方主页）。帖子+回帖共用 `userPostsProvider`（一次性拉全部 kind-1，客户端拆分）。
