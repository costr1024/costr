...
- **本地缓存**：drift/SQLite，社交图谱 gated 持久化（follows + self 的 kind-1/7 落库，
  ...
  **关注流缓存按账号隔离水合**：`events` 表是全设备共享的（各账号关注的人的帖子都落这一张
  表），旧的冷启动水合取「全局最新 1000 条」，多账号时当前账号关注的人会被**别的账号**关注的
  人挤出这 1000 条 → 关注流水合后是空的、只能等 outbox 重新联网拉（「多账号关注流不秒出」）。
  现在水合先读当前账号的 kind-3，只取「本账号关注的人 + 自己」的帖子（`queryFeedForAuthors`），
  无论几个账号，各自的关注流都能秒出。
