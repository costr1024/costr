...
- **音频播放（内联，Amethyst 式）**：正文裸音频链接（`.mp3/.m4a/.aac/.wav/.ogg/.opus/.flac`
  ...
  链接，自动在原文位置渲染播放器卡片：波形条是主体——imeta 带 `waveform` 字段（NIP-A0 语音
  消息，空格分隔的幅度值）画真波形，否则按 URL 种子生成固定形状的合成波形（同一条音频永远
  同一形状，Amethyst 同款做法）；播放进度给波形染色，点按/拖动波形定位，播放/暂停 + 倍速
  （复用视频底部 sheet）+ 分享。点播放才开始加载（不预载），滚出屏幕即释放。引擎
  `just_audio`（`video_player` 官方不支持纯音频）；Windows/Linux 没有音频后端——与这两端
  本来也没有视频播放一致——降级为下载 chip。
