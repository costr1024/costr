...
5. **性能从设计阶段就守**。订阅有界、去重、聚合优先、批量更新、后台不持有 WebSocket、
   全量事件流 默认不验签——见 [性能约束](docs/DESIGN.md#10-性能约束设计阶段就要守)。
...
### 发件箱（outbox）与收件箱（inbox）

![你刷的帖子/评论/点赞/转发来自对方的发件箱；别人回复/@你则写进你的收件箱](docs/img/outbox-inbox.svg)
...
- **全球 tab**：`feedSubscriptionProvider` → `pool.request({kinds:[0,1,6,7], limit:200},
  ...
  **语言过滤开启时窗口按语言保留**：满 1000 条的驱逐优先丢「最旧的非目标语言帖」，
  目标语言帖可累积满整个上限（1000 条配额按语言统计，不再被全量事件流里占绝对多数的外语帖
...
- **个人主页**：`userPostsProvider` → `fetchFromUrls` 临时连接对方 outbox read 中继
  ...
  **历史帖回翻**：帖子/回帖列表滑到最老一条时自动加载更早的一页——`userPostsPagerProvider`
  以当前可见最老帖的 `created_at` 为 `until` 游标向前翻页（同样走对方 outbox、无清单回落广播），
  逐页并入列表；翻空中继后显示「没有更多了」，下拉刷新可重新尝试。深翻出来的旧帖只写 SQLite
  缓存（供点开详情秒开），不进信息流的内存事件池，避免把某人几年前的帖子灌进「关注」首页流。
...
- **帖子详情 / 回帖线程**：`threadAncestorsProvider`（上级链）按 NIP-10 `e`-tag 并行 BFS 上溯，
  ...
  （`threadReplies`：父子树深度优先、同层最旧在前）展示而非扁平 createdAt 倒序。层级用
  **X 式小缩进 + 竖线**：每级仅缩进 10px、最多 4 级封顶（旧实现 `深度*24px` 无上限，
  层级一多内容被挤成一条窄缝）；二级及以下回帖的 `EventCard` 左缘画一条**通高竖线**
  （独立线槽，绝不盖住头像）作为该层级的对齐参考，相邻兄弟回帖的竖线首尾相接成连续一条，
  深帖树也清晰可读。
  ...
  **线程根以 NIP-10 `root` 标记为准**：向上走 replyTo 链可能在「只用 mention 引用主贴、
  自身不算回复」的中间帖处断链（其 replyToId 为空）；此时聚焦回帖自带的 `root` e tag
  仍指向真根——解析该标记并置顶，回帖列表按最终根 `#e` 取数，会话完整。发帖回复的
  root 标签按同一规则在发送前解析（`threadAncestorsProvider`，4s 封顶）。
...
- **通知聚合**（`notificationsProvider`）：`#p` 提及 + `#e` 互动按 `type:target` 聚合成 X 式
  ...
  **聚合人数不再错报**：条目保存**全部**互动作者（旧的 5 人上限 + `extraCount` 每来一人就 +1，
  5 人回复会渲染成「5 人和另外 4 人」、读起来像 9 人）；现在标题直接显示前 3 个名字 +
  「和另外 N 人」，N 由 `pubkeys.length` 精确推出。
  **「整合通知」开关落地**（DESIGN §5.3）：设置 → 通知设置里的开关此前是摆设（硬编码 `true`、
...
- **媒体加载**（头像/帖子图片/视频）：`CostrNetworkImage` 走带超时的 `flutter_cache_manager`
  ...
  （设置 → 代理媒体开关，**不同步中继**）：开启后凡含图/视频/音频链接的帖子在昵称右侧常显「代理媒体」按钮
  （无媒体不显示），点按切换该帖图/视频/音频走 `https://proxy.bostr.online/<origin-domain>/<path>`
  ...
  **头像/背景图被墙也能手动走代理**（仅在「代理媒体」开关开启时）：所有头像（含帖子列表里的
  小头像）与资料页背景图用 `ProxyableNetworkImage`（手动、非自动——与帖子「代理媒体」同款
  逻辑）：先直连原图，被墙类失败（超时/连接被重置，404 不算）时在占位图上浮现「代理」小按钮，
  点按改走代理镜像并**落本地磁盘缓存**。同一 host 一旦学到被墙（按**域名**记忆，会话内生效），
  其余该域名的头像/背景图直接走代理、不再重复 8s 超时——代理过的头像因此也会出现在帖子列表里。
  「代理媒体」开关关闭时，头像/背景图退回普通加载（首字母/渐变占位兜底），不出现代理按钮。
  **tag 声明的媒体按 MIME 渲染**：`["video", url, mime]` / `["audio", url, mime]` / imeta tag
  声明的 URL，即使裸链没有 `.mp4` 等文件扩展名（如抖音分享链 `…/playwm/?video_id=…`），
  也按 tag 的 MIME 渲染内联播放器/音频播放器/九宫格（此前只当普通可点链接，需跳浏览器；对齐 Amethyst）。
...
- **关注集分组**（`_buildFollowGroups`）：按 kind-30000 的 **`d` tag**（稳定标识）分组，组名取该 d 下
  ...
  **组成员 = 列表全量成员**（对齐 Amethyst/NIP-51）：自定义分组成员取**最新修订的全部 `p` 标签**
  （去重、保序）——NIP-51 列表与 kind-3 关注相互独立，列表里可以有当前未关注的人；此前显示
  「列表成员 ∩ kind-3 关注」交集，56 人的列表只显示 39 个（实测各分组全部「少很多」）。成员只从
  最新修订取——旧修订的并集会让已被新版移除的成员复活。默认分组仍是「不在任何列表里的关注」。
  资料页「关注」总数改为直接取 kind-3 的 p 标签数（本人取 `followingStateProvider` 实时列表），
  不再按分组求和（跨组重复关注会被重复计数：73≠70）；`userFollowsProvider` 加 createdAt 新胜守卫，
  慢中继的旧 kind-3 后到不再把数字打回去。
...
- **首页分页**：滚动到底触发 `_loadMore`（global 广播 `until` / following 走 outbox `until`），底部加
  ...
  自动连续向前翻页**：目标语言帖在全量事件流里占比极小，单页 `until`（200 条）几乎翻不到，
  ...
  配合上面的按语言保留，翻出来的目标语言帖不会再被全量事件流挤掉。
...
- **本地缓存**：drift/SQLite，社交图谱 gated 持久化（follows + self 的 kind-1/7 落库，
  全球 全量事件流 不落库），replaceable 事件恒缓存；冷启动 缓存回填 秒出。
  **关注流缓存按账号隔离回填**：`events` 表是全设备共享的（各账号关注的人的帖子都落这一张
  表），旧的冷启动回填取「全局最新 1000 条」，多账号时当前账号关注的人会被**别的账号**关注的
  人挤出这 1000 条 → 关注流回填后是空的、只能等 outbox 重新联网拉（「多账号关注流不秒出」）。
  现在回填先读当前账号的 kind-3，只取「本账号关注的人 + 自己」的帖子（`queryFeedForAuthors`），
  无论几个账号，各自的关注流都能秒出。
  **启动不再等中继连接**：`bootstrapProvider` 此前 `await pool.connect()`——`Future.wait`
  等全部中继 WS/TLS 握手（每个上限 10s，一个被黑洞的中继就把整个启动卡满 10s），期间
  白屏转圈、本地缓存根本没机会渲染。现在身份 + 服务器列表就绪即完成 bootstrap、router
  立即挂载、首屏直接从 SQLite 缓存渲染，中继在后台连接、事件增量进来；连接前发出的
  REQ 由池的 `_activeSubs` 记录、中继连上后 `_resendActive` 自动补发，无竞态。依赖连接
  的 kind-10002 重发/草稿重试挂到后台连接完成后触发（避免零连接时发布失败）。
...
**v1.2.7 相对 v1.2.6 的新增**：
1. **图床服务器可以自己排顺序了**——发帖带图片/视频时，应用按图床列表的顺序逐个
   尝试上传，谁先成功用谁。现在「设置 → 服务器节点 → Blossom 图床 → 自定义」里
   可以按住每行左侧的拖动符号上下拖动，把快的、稳定的服务器排在最前面优先使用，
   点「保存」后发帖上传立即按新顺序走。
2. **一键测出每台图床的真实上传/下载速度**——同一个自定义面板里新增「测速」按钮
   （在「恢复默认」右边），点一下逐台测所有图床：给每台上传一个 10 MB 的测试文件
   再下载回来，分别计时，每台服务器下方直接显示「上传 X.X MB/s · 下载 X.X MB/s」，
   照着结果把快的拖到最前面即可；连不上的显示「测速失败」。测速用完的测试文件会
   自动尝试删掉，不在服务器上留垃圾。两点说明：测速需要登录（上传要用你的身份
   签名）；测速结果不保存，关掉面板就清空、随时可重测，所以数字只代表当时的网络
   状况。

**v1.2.6 相对 v1.2.5 的修复**：
1. **关注信息流停更后能自愈了**——此前中继节点抖动/宕机（如 2026-08-30 上午
   bostr 系节点故障）时，关注页几条「长期连着」的订阅会被服务器端悄悄断开，
   应用察觉不到、也不会重连，关注页就整体停更：自己新发的帖、别人新发的帖都
   刷不出来，下拉刷新无效，只能重启应用或把应用放后台很久等系统断开重连。
   现在加了三道自愈：①服务器主动断开订阅时，应用几秒后自动重发；②每分钟给
   连接做心跳体检，超过 3 分钟没收到任何数据的连接强制重连并重发全部订阅；
   ③关注页每 2 分钟自动补抓一次「这期间漏掉的帖子」（自己的和关注人的）。
   此后节点再抖动，最多几分钟自动恢复，不再需要重启。
...
2. **发帖页 18禁（NSFW）开关改成 18+ 图标**——灰色＝关、红色＝开，点一下切换，宽度固定。
   修复了此前文字开关（尤其打开时）太宽、把右边的帖子字数挤出屏幕显示不全的问题；
   并修掉个别机型上「emoji 灰度化」实现导致整页变灰、盖住输入框与附件按钮的渲染 bug。

**v1.2.3 相对 v1.2.2 的修复/新增**：
1. **帖子里的音频能直接听了**——此前帖子里的音频链接（.mp3、语音等）只能点一下跳
   浏览器或下载，现在自动变成内联播放器卡片：播放/暂停、±10 秒、真进度条（拖动
   定位）、倍速、分享，波形条随进度染色；自己上传的音频发出后也自动显示为播放器。
   同一时刻最多一条音频在播（播新的自动停旧的），App 切到后台自动暂停。Windows/
   Linux 桌面与视频一样暂不支持播放，音频显示为下载按钮。
...
3. **修复附件数量能超出上限的问题**——此前图片正在上传时再打开一次选择，可能超出上限
   ...
   置灰，上传完成即可继续添加。
4. **修复回复/回帖链显示不全**——①回复发出后立即显示在主贴下方；②打开任意一条回帖
   都能看到「从主贴到全部回复」的完整帖子链，此前链上若有「只引用主贴、不算回复」的
...
1. **服务器节点页按 NIP-65 角色拆「发件箱/收件箱」两区**——此前收件箱区只含 `bostr/inbox`
   一台，概念呈现不完整。现 `isInboxRelay` 改为「所有 write 中继」（除只读 `bostr.online`）、
   新增 `isOutboxRelay`「所有 read 中继」（除只写 `/inbox`），均与 kind-10002 标记同源判定；
   「中继服务器」改名「发件箱中继（outbox）」与「收件箱中继（inbox）」对应，读写双角色的中继
   两区都出现。解释文案同步更新，并新增「发件箱/收件箱」原理图（`OutboxInboxDiagram`，
   CustomPainter 深色自适应，`docs/img/outbox-inbox.svg` 同语义）。
2. **回复/发帖偶发卡几秒修复**——两个根因（同一触发条件「没有中继立刻接受」）：①全 NIP-42
   auth 拒绝轮原本固定等满前台重试延迟（默认 1s）才重发，而 AUTH 握手已并发完成，现封顶
   300ms；②「已连接但静默」的僵尸中继使发布轮干等满 5s 窗口，现加 1.5s「无进展」提前收敛。
   成功路径不变（首个接受仍即时返回），慢中继被重发后回 `duplicate` 仍判成功。
...
1. **关注流缓存按账号隔离回填**——此前 `events` 表全设备共享、冷启动取「全局最新 1000 条」，
   多账号时当前账号关注的人被别的账号关注的人挤出这 1000 条，关注流过滤后是空的、只能等
   outbox 联网重拉（「多账号关注流不秒出」）。现在回填只取「本账号关注的人 + 自己」的帖子
...
1. **1000 条窗口配额改为按语言统计**：全球流的纯内存临时窗口上限 1000 帖，此前无差别按时间
   挤掉最旧帖——全量事件流里中文是极少数，几分钟就被外语多数挤光。现在语言过滤开启时，超限驱逐
...
2. **下刷自动连续向前翻页**：目标语言帖占比极小，单页 200 条几乎翻不到，现在翻不到就自动继续
   ...
   位置，切换语言立即解锁新方向的深挖。翻出来的目标语言帖由第 1 条保住，不会再被全量事件流挤掉。

**v1.1.5 相对 v1.1.3 的修复**（版本号跳过 1.1.4，避讳数字 4）：
**点帖子/主页上的 #标签跳首页过滤，也能翻到全部历史了**——此前「点标签过滤」和「关注下拉标签」
是两条路径：下拉标签自 v1.1.2 起已真查中继，但**点帖子/主页上的 #标签**（`tagFilterProvider`，
任意 mode）仍是纯客户端过滤已加载的全球火水/关注流——火水窗口只装得下最近一两天，自然「只有
最近几天」。现两条路径共用 `tagTimelineFeedProvider`：选中任一标签即向默认池广播 live `#t` 订阅
（`#t` 带大小写变体、`kinds [1,6]`、`limit 100`、不带 `since`），信息流来源切换为该标签自己的
时间线——全球 mode 事件入临时窗口、关注 mode 入内存库，对应的火水 / outbox 拉取暂停，上滑按
`until` 回翻。关注的标签、点开的标签，现在都能翻到各中继的完整历史（不支持 `#t` 的中继只是不
应答）。
...
重启之所以能临时修好，是冷启动会把 SQLite 里**全部**缓存资料重新加载进内存，用一阵子逐出又发生。
...
**切换为乐观更新**：内存状态先翻转（「当前」徽标与整条身份链立即响应），安全存储写入
进后台串行队列——此前切换要等 Keystore 写完才更新状态，慢 Keystore 设备上点按后要干等
数秒且毫无反馈。登录/移除仍为先落盘后翻状态（半生效会错报设备私钥状态）；串行队列保证
慢的切换写入不会落在其后的写入之后复活旧状态。
...
菜单（到顶部仍会，触屏优先的取舍）。
**全球 tab 下滑卡顿优化**——全球流是不间断 全量事件流，事件仓库每 200ms 批量发一次
...
**已读通知反复「复活」根治**——v0.8.7 只修了冷启动的 缓存回填 时序竞态，还剩两个根因：
...
「本地缓存 缓存回填」存在竞态：relay 先到时，同一事件按空的自己的帖子快照分类，通知项
id 与 缓存回填 后不同（`mention:<事件id>` vs `reply:<自己帖子id>`），下次启动持久化已读集
合匹配不上，已读的 @/回复通知又带未读黑点、徽标重计（「每次升级版本出现重复」的观感）。
现通知生成器先等事件库 缓存回填 完成再订阅/分类，id 跨启动稳定；并修两个读状态小竞态：
缓存回填 完成前的「全部已读」被旧持久集覆盖、DB 未开时防抖写被静默丢弃（含 dispose 时
flush 未决写）。
...
- **用户主页**：NestedScrollView（banner/头像可滚走 + TabBar 吸顶）+ sliver 根治 overflow。4+1 tab：帖子/回帖/关注/关注者/收藏。`userPostsProvider` StreamProvider：先 yield 内存+SQLite 快照，后台 NIP-65 outbox 定向拉取（250ms 去抖流式 yield），拉取经 `yield* ctrl.stream` 吐给 UI——**修复了两处「有的用户只能看到几条帖子/回帖、下拉刷新也刷不出更多」**：① 原实现把中继结果写进 `ctrl` 却没有 `yield*` 管道，全部静默丢弃、主页永远只显示缓存快照；② 原 `since` 增量以「缓存里最新一条」为锚点，而社交圈外用户的帖子不落库、缓存只是信息流里瞥见的几条，锚点之后的历史永远拉不到——现改为每次拉最新 `limit:100` 窗口（merged 按 id 去重）。**历史帖回翻**：帖子/回帖列表滑到最老一条自动向前翻一页（`userPostsPagerProvider`：以当前可见最老帖 `created_at` 为 `until` 游标、同样走对方 outbox、翻空显示「没有更多了」、下拉刷新 `retry()` 重新武装；两 tab 合并同一 pager，深翻旧帖只写 SQLite 不进信息流事件池，避免灌进关注首页流）。下拉刷新、发帖后立即可见。各 tab 的过滤/搜索框（搜帖子/搜回帖/过滤已关注/过滤关注者/搜收藏）共用一个 `_SearchBar`，带 **X 一键清空**——清空文本同时回调 `onChanged('')` 复位父级 `_query`，列表回到全量（`controller.clear()` 本身不触发 `TextField.onChanged`，须手动回调）。
...
- **音频播放（内联，Amethyst 式）**：正文裸音频链接（`.mp3/.m4a/.aac/.wav/.ogg/.opus/.flac`
  等）、imeta `m audio/*` 附件、`["audio",url,mime]` tag、以及探测响应为 `audio/*` 的无扩展名
  链接，自动在原文位置渲染播放器卡片。控件全部显式可点：播放/暂停、±10 秒、真进度滑条
  （拖动暂停、松手定位）、倍速（复用视频底部 sheet）、分享；波形条是进度可视化——imeta 带
  `waveform` 字段（NIP-A0 语音消息）画真波形，否则按 URL 种子生成固定形状的合成波形
  （Amethyst 同款做法）。点播放才开始加载，滚出屏幕即释放；**同一时刻最多一条音频在播、
  App 切后台全部暂停**（信息流音频跟随可见卡片，不做后台播放）。引擎 `just_audio`
  （`video_player` 官方不支持纯音频）；Windows/Linux 没有音频后端——与这两端本来也没有
  视频播放一致——降级为下载 chip。
...
- **自定义关注列表（NIP-51 kind-30000）**：兼容 Amethyst 的 `d`=UUID + `name`=人类名约定；`kind30000DisplayName` 优先 `name` 回退 `d`；编辑保留 UUID `d`+元数据。**重命名**（⋯ 菜单 → 底部弹窗，`d` 不变只改 `name`，列表不分叉）+ **删除**（发 NIP-09 kind-5 `a`-坐标 `30000:pubkey:d`，删全版本——Amethyst 自带删除只发 `e` 对 replaceable 无效，这是修正）。新列表创建用 UUID `d`（Amethyst 约定，避免重命名后 `d` 冲突）。组成员行与计数均显示列表**全量成员**（最新修订 `p` 标签去重，非「已关注 ∩ 组内」交集——列表可含未关注者，对齐 Amethyst）。
- **服务器节点页**：四类服务器——中继（按角色拆成 **发件箱/收件箱** 两区）/搜索中继（NIP-50）/索引中继/Blossom 图床（连接状态 + 真实 WS/HTTP RTT，NIP-50 CLOSED 回退 search 重试）。中继列表按 **NIP-65 角色**拆成两区：**「发件箱中继（outbox）」**＝read 中继（你的帖子所在、别人来取），**「收件箱中继（inbox）」**＝write 中继（别人回复/@你/私信你时投递给你）；两区共用同一份列表与「自定义」入口，读写双角色的中继两区都出现，只读的 `bostr.online` 只在发件箱、只写的 `bostr.online/inbox` 只在收件箱（`isOutboxRelay`/`isInboxRelay` 与 kind-10002 标记同源判定）。页面配一张「发件箱/收件箱」原理图（`OutboxInboxDiagram`，`docs/img/outbox-inbox.svg` 同语义）。时延缓存 SQLite，颜色绿快黄慢红离线。**每类标题行右侧「自定义」**（高级功能，带劝退警示）：增删服务器（每类最多 10 台，中继至少 3 台、其余至少 1 台）、恢复默认；列表本地持久化、全账号共享。中继变更即热替换连接池，并以当前账号发布 kind 10002 中继更新事件；非活跃账号切换到前台时比对标记补发，失败自动重试。图床变更纯本地，发帖上传按自定义列表顺序重试。**「为你推荐」（去中心化）**：候选来自用户自己视图里他人发布的服务器列表（kind 10002/10063）聚合投票，不依赖中心服务器；每个候选实测通过才显示——中继须可连接 + 免费（NIP-11），搜索中继另须自声明支持 NIP-50，图床须实测可免费上传（检测会上传一个极小测试文件），未登录不推荐图床；无可推荐则不显示，结果缓存 24h。「换一批」是**滚动推荐**：去重只在一个周期内生效——已推荐过的 URL 记入轮转记忆（`server_reco_seen:<类别>`，持久化、封顶 50 条），刷新时排除以优先展示新面孔；可一旦没有「新的可推荐」服务器——候选全都展示过、或仅剩的候选探测不通过——就回绕到最初那批重新开始循环，绝不因换尽而让用户长期看到空白。面板文案明确提醒「检测只代表当时，更换后需自行验证可用性」，搜索中继（搜索效果无法自动检测）与索引中继（无推荐块，提醒放在劝退警示里）尤重。**写成功率统计**：记录每台写中继最近 10 次发布的显式裁决（超时不计、读路径不测），成功不足一半时节点页标红「建议更换」，点按进入自定义面板更换。
...
- 到达事件**不做** Schnorr 验签（全量事件流 性能）；`Event.isSignatureValid` hook 已接线但默认关闭。
