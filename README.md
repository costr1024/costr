...
| 平台 | 版本 | 文件 |
| --- | --- | --- |
| Android | **0.8.8-beta** | [`app-release.apk`](https://github.com/costr1024/costr/releases/download/v0.8.8-beta/app-release.apk)（≈72 MB，universal APK，含 arm64/arm/x86_64/x64 全 ABI） |
...
`test/` 下 464 个测试覆盖：纯协议层（bech32/nip19/nip44/identity/event）、relay 池与
...
弹出。根因：隐藏动画 220ms 内视口长高 ~200px，快速甩动（ballistic）撞上视口变化时
Flutter 不做静默边界钳制而是重启甩动；关注列表短、甩到底附近时位置超出新的边界，
系统用**弹簧把偏移拉回**，回拉产生「无手指细节的负位移增量」滚动通知，被方向检测
误判成「用户在上滑」→ 菜单栏弹出，用户继续下滑又隐藏 → 反复振荡（慢拖走静默钳制、
无通知，所以慢滑没事；全球 tab 列表长、甩不到底，所以只在关注出现）。现隐藏后的
500ms 窗口内，「无手指细节的负增量」更新一律忽略（不判显示也不累计）；真实手指
拖拽与回到列表顶部的显示逻辑不受影响。
...
另从 SQLite 稳定补全「自己的帖子」快照（跨启动一致）；关注门**超时不再误判新关注**
（慢中继此前会把老关注者改列表当「开始关注你」重新通知，现超时按跳过处理）；「全部
